...
LAVASNEK_PREFIX = os.environ["LAVASNEK_PREFIX"]
...
async def play(ctx: lightbulb.Context) -> None:
    ...
    try:
        ...
        await ctx.respond(f"Use `{LAVASNEK_PREFIX}join` first")
        return
...
async def skip(ctx: lightbulb.Context) -> None:
    ...
    if not skip:
        ...
        # start playing (because there isn't any),
...
async def data(ctx: lightbulb.Context) -> None:
    ...
    If just `data` is ran, it will show the current data, but if
    `data <key> <value>` is ran, it will insert that data to the
    node and display it.
    """
